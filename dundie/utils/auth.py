        ...
        if not email or not password:
            ...
            if click.confirm("Authentication required. Do you want to login?"):
                ...
                if not is_admin:
                    ...
                    if role:
                        click.secho(
                            f"   User found but without permission.",
                            fg="white",
                        )
                        click.secho(f"   Your role: {role}", fg="yellow")
                        click.secho(
                            f"   Required roles: Manager, CEO, Admin, C-Level",
                            fg="yellow",
                        )
                    else:
                        click.secho(
                    ...
                    click.secho("=" * 50, fg="red")
                    click.secho(
                        "\n   Only users with Manager, CEO, Admin, or C-Level role",
                        fg="white",
                    )
                    click.secho(
                        "   can perform administrative operations like removing points.\n",
                        fg="white",
                    )
                    return
...
def require_password_verification(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        with get_session() as session:
            user = session.exec(select(User).where(User.person_id == current_user.id)).first()
