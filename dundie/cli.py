...
from dundie.utils.log import get_logger  # noqa: E402
    ...
    try:
        ...
        if result:
            ...
            for person in result:
                ...
                table.add_row(
                    person["name"], person["dept"], person["role"], person["email"], person["currency"], status
                )
...
def add_cmd(email: str, value: int, current_user=None):
    ...
    try:
        ...
        if balance_info:
            ...
            console.print(
                f"[cyan]Novo saldo de {balance_info['name']}: "
                f"{balance_info['balance']} {balance_info['currency']}[/cyan]"
            )
        else:
            console.print(f"[green]✅ {value} pontos adicionados com sucesso![/green]")
...
def remove(email: str, value: int, current_user=None):
    ...
    try:
        ...
        if current_balance < value:
            console.print(
                f"[red]❌ Saldo insuficiente! {balance_info['name']} " f"tem apenas {current_balance} pontos.[/red]"
            )
            return
        ...
        if new_balance_info:
            ...
            console.print(
                f"[cyan]Novo saldo de {new_balance_info['name']}: "
                f"{new_balance_info['balance']} {new_balance_info['currency']}[/cyan]"
            )
        else:
            console.print(f"[green]✅ {value} pontos removidos com sucesso![/green]")
...
def transfer_cmd(email: str, value: int, current_user=None):
    ...
    if not balance_info or balance_info["balance"] < value:
        console.print(
            f"[red]❌ Saldo insuficiente! Você tem " f"{balance_info['balance'] if balance_info else 0} pontos.[/red]"
        )
        return
...
def statement(email: Optional[str] = None, limit: int = 10, current_user=None):
    ...
    try:
        ...
        for mov in movements:
            ...
            # Cor do valor
            value_color = "green" if mov["value"] > 0 else "red"
...
def login():
    ...
    if authenticate(email, password):
        ...
        if is_admin:
            ...
            console.print("[blue]🔹 Privilégios: Usuário Padrão[/blue]")
